...
- [x] Persist complaint/work type/description/instructions, scheduled/due/estimated duration, priority, signature policy and reference photo/document attachments with customer/site/optional target. (Verified: `CreateTaskDto` now also has `workType` (free-form string — PREVENTIVE/CORRECTIVE/COMPLAINT/INSPECTION offered as options in the frontend, not DB-enforced) and `signaturePolicy: { required: boolean }`, added via migration `031-sprint-10-task-worktype-signature.sql` (`tasks.work_type text`, `tasks.signature_policy jsonb`, additive only). `TaskRepository.create`'s INSERT and `TASK_SELECT` both updated; `create-task-form.tsx` gained a work-type select and a "require signature" checkbox. Live-verified against the running stack: `POST /tasks` with `workType`/`signaturePolicy` round-tripped correctly through the real DB. Reference attachments were already real (`POST /tasks/:id/attachments`, `TaskAttachmentService`). Covered by 2 new backend tests (`task.service.create.spec.ts`) and 1 new frontend test; full backend suite (138 tests) and frontend suite (26 tests) green.)
...
- [ ] Represent overdue, escalated, customer-unavailable, safety-stop and sync-pending as typed flags/exceptions, never substitute statuses; make post-approval cancel/reopen policy explicit. Partial, 3 of 5 flags now real: `task-flags.ts` computes `OVERDUE`/`ESCALATED` purely from `dueAt`/`status`/`now` (never persisted — a stored overdue flag goes stale the instant time passes it, so it's computed fresh on every `TaskService.get`/`list`, mirroring how `syncHealth` is computed), and `SYNC_PENDING` from real dead-lettered `outbox_events` in the last 24h (`TaskRepository.findDeadLetteredTaskIds`, one batched query, not N+1) — the same signal source as tenant-level `syncHealth`. Flags are additive on top of whatever's already stored in `tasks.flags` (`mergeTaskFlags`), leaving that column as a future explicit-write channel. Surfaced in `list.tsx`/`detail.tsx` with labeled, colored badges. Live-verified: a task created 30h past due returned `["OVERDUE", "ESCALATED"]` from the running stack. `CUSTOMER_UNAVAILABLE` and `SAFETY_STOP` are explicitly NOT wired — both are worker-reported, execution-time states (customer absent at visit, rule-engine `safety_stop` firing during evidence collection) with no real trigger source until the mobile execution flow exists (Sprint 12/13); inventing one now would be a flag with nothing real behind it. Post-approval cancel/reopen policy is also not addressed here. Covered by 14 new tests (`task-flags.spec.ts`, `task.service.flags.spec.ts`).
