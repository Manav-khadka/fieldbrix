...
- [ ] Build platform tenant list/detail/health/usage/provision flows and god-mode entry; persistent banner remains visible on every tenant screen. Backend flows are real (`listTenants/tenantDetail/tenantUsage/createTenant`, `startGodSession`/`endGodSession`), but the frontend god-mode banner (`fieldbrix-frontend/src/App.tsx` `god-banner`) is only mounted inside the Tenants tab, not the app shell — it disappears when navigating to Company/People/Roles. Fix scheduled with the Sprint 06 frontend router/shell bootstrap.
- [ ] Build tenant settings for logo/colors/terms/timezone/locale/date/number formats, working hours, policies and validation preview. Locale/timezone/working-days/working-hours are implemented and validated (`PlatformService.updateCompany`); logo/branding upload, color theme, and date/number-format settings are not yet modeled — not started.
...
- [ ] Expose plan/user/task limits, active/completed counts, evidence volume, last activity and repeated sync/import/inactivity health with append-only support notes/actions. Limits/usage counts/append-only support notes are real (`tenantUsage`, `ensureLimit`, DB `reject_support_note_mutation` trigger); `syncHealth` is still a static default (`'healthy'`/`'inactive'`) not computed from real sync/import/inactivity signals — not started.
