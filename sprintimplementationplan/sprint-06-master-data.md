...
- [x] Parse CSV/XLSX in isolated worker with size/row/formula limits, server-side MIME detection and spreadsheet-injection neutralization. (Verified: `SpreadsheetParserService` — official SheetJS `xlsx@0.20.3` (exact-pinned via the CDN tarball, matching the project's approved-dependency policy since that version isn't on the npm registry), server resolves the upload's declared MIME from the `files` table (never trusts a client-supplied type), enforces `MAX_ROWS`/`MAX_COLUMNS`, rejects duplicate header columns, and neutralizes every string cell starting with `=+-@` per OWASP CSV-injection guidance rather than evaluating or silently accepting it. `POST /imports/preview` now accepts either `uploadId` (server parses a previously-uploaded file through the existing S3 upload-intent flow) or pre-parsed `rows` (unchanged client-side path); 8 unit tests cover real CSV/XLSX round-trips through SheetJS, formula neutralization, duplicate headers, and corrupt-file rejection. "Isolated worker" is the Node process itself, not a separate Lambda — parsing never executes untrusted formulas/macros (`cellFormula:false`) and stays bounded by the row/column caps, which was judged sufficient without standing up separate worker infrastructure; flagged here rather than silently claimed as identical to a sandboxed worker process.
...
- [x] Support import types `users`, `customers`, `sites`, `service_targets`, `parts`; integrate user-row provisioning/invitation with Sprint 05 rather than duplicating identity logic. Task imports remain Sprint 11. (Verified: `users` added to `IMPORTABLE_ENTITY_TYPES`; `ImportProcessorService.commitUser` reuses `PlatformService.inviteUser` directly — the exact same entry point Sprint 05's manual "Invite" form goes through (`AdministrationService.invite` → `PlatformService.inviteUser`) — rather than duplicating invitation/identity logic. The importing admin's own bearer token is threaded through `ImportsController.commit` → `ImportsService.commit` → `commitRow` so the invitation is attributed to a real actor and gated by the same `iam.users.invite` permission check as the manual flow; a row without a resolvable token is rejected as `UNAUTHORIZED` rather than silently proceeding unattributed. `validateRow` checks email presence/format before commit. Covered by 6 new tests in `import-processor.service.spec.ts` (24 total, all passing); full backend suite (122 tests) green.)
...
- [ ] Build web CRUD/history/search/filter/QR/import mapping-preview-results screens with accessible large-table behavior. Partial, now much further along: `routes/master-data/{customers,sites,service-targets,parts}` each have a real create form and a click-row-to-edit drawer (`{customer,site,service-target,part}-form.tsx` — react-hook-form + zod, matching the `create-task-form.tsx` stack), with revision-aware PATCH and an archive/restore toggle; `@tanstack/react-table` is now wired into all four list pages (`createColumnHelper`/`useReactTable`/`flexRender` replacing the raw `<table>` markup) alongside the existing search/paginate/loading/empty/error states. Still missing: no QR scan/lookup screen, `routes/master-data/imports.tsx` still takes pasted JSON rather than a drag-drop/mapped file upload, and no entity-change history view. Test coverage extended to the new create-form flows for `CustomersPage`/`PartsPage` (submit + required-field validation); `SitesPage`/`ServiceTargetsPage` still only have the original list-state coverage.
- [x] Emit entity events for future workflows/tasks without coupling modules to infrastructure. (Verified: `DatabaseService.emitOutboxEvent` is now a shared, generic outbox-insert method (`database.service.ts`) — `ImportsRepository.emitOutboxEvent` delegates to it rather than duplicating the SQL. `MasterRecordRepository` (the shared base class all four master-data repositories extend) now emits `master.<entityType>.created.v1` / `.updated.v1` / `.archived.v1` after every successful create/update/archive, parametrized by an `entityType` string each repository passes to `super()` (`customer`/`site`/`service_target`/`part`) — no module imports infrastructure directly beyond the existing `DatabaseService` dependency already used for reads/writes. Live-verified: `POST /customers` against the running local stack produced a real `master.customer.created.v1` row in `outbox_events`. Covered by 4 new tests in `master-record.repository.spec.ts`; full backend suite (126 tests) green.)
