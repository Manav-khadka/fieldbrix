...
- [ ] Validate unique keys, positions, label/help limits, option duplicates, field configuration and references server-side. Partial: `WorkflowDraftService.addField` requires non-empty key/type/label and a known field type, but does not reject a duplicate key against existing fields at add time (only surfaced later by `validate()`); no label/help length limits, option-duplicate checks, or field-config schema validation exist.
