...
```bash
alias fieldbrix-aws='cd /absolute/path/to/fieldbrix/docs/terraform && source aws.env.local && ./scripts/aws-login.sh'
```

## 4. Verify before Terraform

Run all of these from `docs/terraform`:
...
- [ ] SSH private key exists only at `~/.ssh/fieldbrix_prod`
