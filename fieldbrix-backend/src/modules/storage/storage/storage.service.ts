...
export class StorageService implements OnModuleDestroy {
  ...
  validate(mime: string, size: number) {
    if (!this.allowedMime.has(mime))
      throw new UnsupportedMediaTypeException('UNSUPPORTED_MIME');
    if (!Number.isSafeInteger(size) || size < 1 || size > 25 * 1024 * 1024)
      throw new PayloadTooLargeException('FILE_TOO_LARGE');
  }
  async put(key: string, body: Uint8Array, mime: string) {
    this.validate(mime, body.byteLength);
