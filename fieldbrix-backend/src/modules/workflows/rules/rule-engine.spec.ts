...
import type { Rule, RuleValue } from '../../workflows/rule-engine';
...
describe('evaluateRules', () => {
  ...
  describe('operator: equals', () => {
    it('fires when condition matches', () => {
      const out = evaluateRules(
        [rule({ conditions: [{ fieldKey: 'status', operator: 'equals', value: 'bad' }], actions: [{ type: 'warning', message: 'Alert!' }] })],
        { status: 'bad' },
      );
  ...
  describe('operator: not_equals', () => {
    it('fires when values differ', () => {
      const out = evaluateRules(
        [rule({ conditions: [{ fieldKey: 'x', operator: 'not_equals', value: 'a' }], actions: [{ type: 'warning', message: 'diff' }] })],
        { x: 'b' },
      );
  ...
  describe('operator: greater_than', () => {
    it('fires when numeric actual > expected', () => {
      const out = evaluateRules(
        [rule({ conditions: [{ fieldKey: 'temp', operator: 'greater_than', value: 100 }], actions: [{ type: 'failure', message: 'hot' }] })],
        { temp: 150 },
      );
  ...
  describe('operator: less_or_equal', () => {
    it('fires at equal value', () => {
      const out = evaluateRules(
        [rule({ conditions: [{ fieldKey: 'x', operator: 'less_or_equal', value: 5 }], actions: [{ type: 'warning', message: 'low' }] })],
        { x: 5 },
      );
  ...
  describe('operator: contains', () => {
    it('fires on array inclusion', () => {
      const out = evaluateRules(
        [rule({ conditions: [{ fieldKey: 'tags', operator: 'contains', value: 'urgent' }], actions: [{ type: 'warning', message: 'urg' }] })],
        { tags: ['urgent', 'other'] as unknown as RuleValue },
      );
      expect(out.warnings).toContain('urg');
    });
    it('fires on substring match', () => {
      const out = evaluateRules(
        [rule({ conditions: [{ fieldKey: 'note', operator: 'contains', value: 'broken' }], actions: [{ type: 'failure', message: 'brk' }] })],
  ...
  describe('operator: in', () => {
    it('fires when value is in array', () => {
      const out = evaluateRules(
        [rule({ conditions: [{ fieldKey: 'code', operator: 'in', value: ['A', 'B', 'C'] as unknown as RuleValue }], actions: [{ type: 'warning', message: 'in!' }] })],
        { code: 'B' },
      );
  ...
  describe('operator: is_empty', () => {
    it('fires for null', () => {
      const out = evaluateRules(
        [rule({ conditions: [{ fieldKey: 'x', operator: 'is_empty' }], actions: [{ type: 'warning', message: 'empty' }] })],
        { x: null },
      );
  ...
  describe('action: set_visible / set_required', () => {
    it('tracks visible state per field key', () => {
      const out = evaluateRules(
        [rule({ conditions: [], actions: [{ type: 'set_visible', fieldKey: 'photo', value: false }] })],
        {},
      );
    ...
    it('tracks required state per field key', () => {
      const out = evaluateRules(
        [rule({ conditions: [], actions: [{ type: 'set_required', fieldKey: 'sig', value: true }] })],
        {},
      );
  ...
  describe('priority ordering', () => {
    it('higher priority rule fires first (DESC)', () => {
      ...
      evaluateRules(
        [
          { id: 'low', priority: 1, conditions: [], actions: [{ type: 'warning', message: 'low' }] },
          { id: 'high', priority: 10, conditions: [], actions: [{ type: 'warning', message: 'high' }] },
        ],
        {},
      );
    ...
    it('equal priority resolves by id ASC', () => {
      const out = evaluateRules(
        [
          { id: 'z', priority: 5, conditions: [], actions: [{ type: 'warning', message: 'z-fires' }] },
          { id: 'a', priority: 5, conditions: [], actions: [{ type: 'warning', message: 'a-fires' }] },
        ],
        {},
      );
  ...
  describe('action: require_evidence deduplication', () => {
    it('does not duplicate evidence entries', () => {
      const out = evaluateRules(
        [
          { id: 'r1', priority: 0, conditions: [], actions: [{ type: 'require_evidence', fieldKey: 'photo' }] },
          { id: 'r2', priority: 0, conditions: [], actions: [{ type: 'require_evidence', fieldKey: 'photo' }] },
        ],
        {},
      );
...
describe('validateRules', () => {
  ...
  it('returns valid=true for a well-formed rule', () => {
    const result = validateRules(
      [{ id: 'r1', priority: 1, conditions: [{ fieldKey: 'temperature', operator: 'greater_than', value: 100 }], actions: [{ type: 'warning', message: 'hot' }] }],
      fieldKeys,
    );
  ...
  it('flags unknown field key in conditions', () => {
    const result = validateRules(
      [{ id: 'r1', priority: 0, conditions: [{ fieldKey: 'nonexistent', operator: 'equals', value: 1 }], actions: [] }],
      fieldKeys,
    );
    expect(result.valid).toBe(false);
  ...
  it('flags unknown field key in actions', () => {
    const result = validateRules(
      [{ id: 'r1', priority: 0, conditions: [], actions: [{ type: 'set_visible', fieldKey: 'ghost' }] }],
      fieldKeys,
    );
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.code === 'UNKNOWN_FIELD')).toBe(true);
