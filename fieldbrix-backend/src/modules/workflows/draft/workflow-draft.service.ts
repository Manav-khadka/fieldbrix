...
export class WorkflowDraftService {
  ...
  async update(id: string, dto: UpdateWorkflowDto) {
    ...
    return this.repo.update(id, dto as unknown as Record<string, unknown>, revision);
  }
  ...
  async patchSection(workflowId: string, sectionId: string, dto: UpdateSectionDto) {
    const current = await this.repo.findById(workflowId);
    const revision = dto.revision ?? (current.revision as number);
    return this.repo.patchSchemaElement(workflowId, 'sections', sectionId, dto as unknown as Record<string, unknown>, revision);
  }

  async removeSection(workflowId: string, sectionId: string, revision?: number) {
    const current = await this.repo.findById(workflowId);
    const rev = revision ?? (current.revision as number);
    return this.repo.removeSchemaElement(workflowId, 'sections', sectionId, rev);
  }
  ...
  async addField(id: string, dto: CreateFieldDto, revision?: number) {
    ...
    if (!isKnownFieldType(dto.type))
      throw new BadRequestException(`UNKNOWN_FIELD_TYPE: ${dto.type}`);
    const current = await this.repo.findById(id);
    const rev = revision ?? (current.revision as number);
    const schema = current.schema as Record<string, unknown>;
  ...
  async patchField(workflowId: string, fieldId: string, dto: UpdateFieldDto) {
    const current = await this.repo.findById(workflowId);
    const revision = dto.revision ?? (current.revision as number);
    return this.repo.patchSchemaElement(workflowId, 'fields', fieldId, dto as unknown as Record<string, unknown>, revision);
  }
  ...
  async reorder(id: string, dto: ReorderDto) {
    return this.repo.reorder(id, dto.sectionOrder, dto.fieldOrder, dto.revision);
  }

  async validate(id: string) {
    const workflow = await this.repo.findById(id);
    const errors: Array<{ path: string; message: string }> = [];
    if (!workflow.name) errors.push({ path: 'name', message: 'Workflow name is required' });
    ...
    if (sections.length === 0) errors.push({ path: 'sections', message: 'At least one section is required' });
    if (fields.length === 0) errors.push({ path: 'fields', message: 'At least one field is required' });
