...
describe('computeTimeBasedFlags', () => {
  ...
  it('returns no flags when there is no dueAt', () => {
    expect(
      computeTimeBasedFlags({ status: 'IN_PROGRESS', dueAt: null }, now),
    ).toEqual([]);
  });
  ...
  it('returns no flags when dueAt is in the future', () => {
    ...
    expect(
      computeTimeBasedFlags({ status: 'IN_PROGRESS', dueAt }, now),
    ).toEqual([]);
  });
  ...
  it('returns OVERDUE and ESCALATED when dueAt is more than 24h past', () => {
    const dueAt = new Date(now.getTime() - 25 * 60 * 60_000).toISOString();
    expect(
      computeTimeBasedFlags({ status: 'IN_PROGRESS', dueAt }, now),
    ).toEqual(['OVERDUE', 'ESCALATED']);
  });
