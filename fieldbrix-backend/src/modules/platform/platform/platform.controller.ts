...
export class PlatformController {
  ...
  @Post('files/upload-intents') async uploadIntent(
    ...
    if (!key)
      return this.platform.uploadIntent(
        token,
        body.mime,
        body.size,
        body.checksum,
    ...
    return (
      await this.idempotency.getOrCreateAsync(key, fingerprint, () =>
        this.platform.uploadIntent(token, body.mime, body.size, body.checksum),
      )
    ).response;
  }
  @Post('files/:id/complete') async completeUpload(
    @Headers() headers: Record<string, string>,
    @Param('id') id: string,
    ...
    const token = this.token(headers);
    const key = headers['idempotency-key'];
    if (!key) return this.platform.completeUpload(token, id, body.checksum);
    const fingerprint = this.idempotency.fingerprint(
      'POST',
      `/files/${id}/complete`,
      { id, ...body },
    ...
    return (
      await this.idempotency.getOrCreateAsync(key, fingerprint, () =>
        this.platform.completeUpload(token, id, body.checksum),
      )
    ).response;
  }
  @Get('audit-events') audit(
    ...
    return this.platform.auditEvents(this.token(headers), query);
  }
  @Get('audit-events/verify') verifyAudit(
    @Headers() headers: Record<string, string>,
  ) {
    return this.platform.verifyAuditChain(this.token(headers));
  }
}
